...
class PoolController extends AdminBaseController
{
    ...
    public function addInvite(PersonnelInviteModel $PersonnelInviteModel){
        if( $this->request->isPost()){
            ...
            if(!Db::name('personnel')->where(['pid'=>$this->request->param('pid')])->value('uid')){
                $this->error("邀约失败 ,该简历无归属人！");
            }
    ...
    public function preliminaryTest(){
        ...
        if(isset($request['export']) && $request['export']){
            $data = DB::name('personnel_invite')
                ->alias('a')
               // ->distinct(true)
                ->field('a.*,a.create_time as pritime,b.*,c.*,d.user_nickname as blang,f.job as apply_job,
...
//echo $where;die;
    ...
    public function addTrain(){
        ...
        if(isset($request['iid']) && $request['iid']){
            ...
            $re = Db::name('personnel_invite')->where(['pid'=>$invite['pid']])->order('iid desc')->find();
            if(!$re['departid']){
                $this->error("邀约失败！没有分配部门");
            }
            if($invite){
    ...
    public function train(){
        ...
        if(isset($request['export'])  && $request['export']){
            $data = Db::name('personnel_invite')
                ->alias('a')
                ->field('a.*,c.*,b.user_nickname,b.user_login as code,f.train_date,f.tid,f.is_pass,f.update_time,e.p_name,e.p_telphone,g.start_time,g.periods,g.end_time,g.periods')
                ->join('user_department c','a.departid=c.id','left')
    ...
    public function induction(){
        ...
        if(isset($request['induction_date']) && $request['induction_date']){
            $where .= $where ? " g.induction_date like '".$request['induction_date']."%'" : "g.induction_date  like '".$request['induction_date']."%'";
        }
        ...
        if(isset($request['export']) && $request['export'] == 1){
            ...
            $data = Db::name('personnel_train')
                ->alias('a')
                ->field('a.*,c.*,b.user_nickname,f.*,g.induction_date,g.code,
                e.interview_status,e.p_name,h.card,h.email,h.phone,h.channels,h.is_recommend,h.recommend_user,
                b.code as pcode
                ')
                ->join('user_department c','a.departid=c.id','left')
                ->join('personnel e','a.pid=e.pid','left')
    ...
    public function setPeriods(){
        ...
        if( $tid &&  $periods){
            ...
            $now    =   time();
            $new_start  =   strtotime($start)-9*60*60;
